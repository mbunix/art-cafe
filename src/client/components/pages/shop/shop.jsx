...
import { Card, Col, Container, Row,ListGroup,Image,Button} from 'react-bootstrap';
...
const Shop = ({ history,match }) => {
  ...
  const userLogin = useSelector((state) => state.artworkDetails)
  ...
  }, [dispatch, match, sucessArtworkReview])
  const addToCartHandler = () => {
    history.push(`/cart/${match.params.id}? quantity =${quantity}`)
  }
  const submitHandler = (e) => {
    e.preventDefault()
    dispatch(createArtReview(match.params.id, {
      rating,
      comment
    })
    )
    ...
    return (
      <>
        <Link className="btn btn-light my-3" to="/"><i className="fas fa-arrow-left"></i> Go back</Link>
        {loading ? (
          <Loader />
        ) : error ? (
                    ...
                    <Col md={3}>
                      ...
                      <ListGroup.Item>
                        <Button onClick={addToCartHandler}
                          className='btn-block'
                          type='button' disabled={artwork.countInStock === 0}>Add to Cart
                        </Button>
                          </ListGroup.Item>
                      </ListGroup>
                      </Card>
